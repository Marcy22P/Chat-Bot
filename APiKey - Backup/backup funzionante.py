...
# Inizializza il client OpenAI per il tuo assistente specifico
    ...
    if "message" in data:
        ...
        try:
            response = assistant_client.chat.completions.create(
                model="asst_d3oUbNFuuoKzsuggRHde6g3N",  # Sostituisci con l'ID del tuo assistente
                messages=[
...
@app.route('/assistant-playground', methods=['POST'])
def assistant_playground():
    data = request.json
    app.logger.info("Ricevuta una richiesta JSON per l'assistente nel playground: %s", data)
    
    if "message" in data:
        message_value = data["message"]
        
        try:
            # Invia la richiesta al tuo assistente nel playground
            response = requests.post(
                ...
                json={
                    ...
                    "model": "asst_d3oUbNFuuoKzsuggRHde6g3N",  
                }
            )
            ...
            app.logger.error("Si è verificato un errore con l'assistente nel playground: %s", str(e))
            return jsonify({"status": "Errore nell'interazione con l'assistente nel playground"}), 500
    else:
        return jsonify({"status": "Nessun messaggio trovato per l'assistente nel playground"}), 400
